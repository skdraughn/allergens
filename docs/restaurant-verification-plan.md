...
2. Mark items without found official allergen evidence as `allergenSourceType: "unavailable"`. When a complete official matrix supplies an explicit item row, retain a compact restaurant-level `officialAllergenProfiles` entry and the item's `officialAllergenProfileId`; a blank row is negative evidence only for that profile's declared allergens.
...
6. At display time, `src/lib/safety.ts` resolves coverage allergy by allergy. Ingredient Intelligence is consulted only for selected allergens outside the item's official profile. It may raise `caution`; it never overrides direct official evidence.
...
6. **Apply direct allergen evidence.** Preserve explicit restaurant-issued or linked-vendor positives and cross-contact statements with their authority labels. Never infer that an allergen is absent from a menu description, culinary expectation, or missing matrix. A blank cell or negative marker may be retained only when the item has an explicit row in a complete official matrix whose covered allergen columns are recorded in an official allergen profile. Allergens outside that profile remain unavailable.
7. **Reconcile every frozen item exactly once.** Assign each frozen `auditItemKey` one disposition: `exact_match`, `normalized_match`, `equivalent_presentation`, `stale`, `artifact`, `location_mismatch`, or `unresolved`. Link matches to current product keys and evidence IDs. The reconciled key set must equal the frozen key set with no omissions or duplicates.
   - Treat section headings, slogans, prices, serving-size fragments, preparation prose, and policy text as artifacts unless the current menu independently presents them as orderable products. Explicitly check the surrounding source for genuine products hidden by those fragments.
   - A current-product rebuild may not wholesale-reset frozen direct positives to unavailable. Preserve each restaurant-issued or restaurant-linked positive that remains supported, or document the item-level reason it no longer applies.
   - Matrix-search attempts must record actual inspected URLs or complete queries, outcomes, and resolving evidence IDs. A class label, empty URL list, homepage placeholder, or unproven vendor link does not complete a search class.
