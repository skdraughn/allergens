...
const id = "osm-bibbop-7802068505";
...
assert(result.currentProducts.every((p) => !p.containsAllergens.length || (p.allergenSourceType === "restaurant_allergen_document" && p.allergenAuthorityTier === "restaurant_issued" && p.allergenSourceEvidenceIds.length === 1 && p.allergenSourceEvidenceIds[0] === "E2")), "direct claim provenance changed");
...
assert(current.some((s) => s.surfaceId === "official-menu") && current.some((s) => s.surfaceId === "official-allergen-pdf"), "official menu/PDF not complete/current");
