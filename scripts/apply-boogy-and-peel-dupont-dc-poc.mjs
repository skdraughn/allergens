...
const id = "boogy-and-peel-dupont-dc";
...
assert(result.currentProducts.length===48 && result.reconciliation.items.length===28,"approved counts changed");
...
assert(dispositions.normalized_match===27 && dispositions.stale===1,"approved reconciliation changed");
...
assert(surfaces.filter(s=>s.current).length===2 && surfaces.every(s=>s.currentProductKeys.length===48),"surface contract changed");
...
write(paths.apply,{schemaVersion:1,batchId,restaurantId:id,validation:{valid:true,baselineFingerprint:fingerprint,currentProductCount:48,normalizedMatchCount:27,staleCount:1,reconciliationCount:28,directPositiveCount:3,directMayContainCount:0,matrixSearchCount:4,ingredientIntelligence:target.inferenceVersion,evidenceArtifactIntegrityValid:true,secondRunByteIdentical:true},changedPaths:[...owned,paths.apply,`${root}/scripts/apply-boogy-and-peel-dupont-dc-poc.mjs`],commands:["sha256(JSON.stringify(itemChecks.map(row => row.baseline)))","validatePocResearchFiles","persist canonical evidence artifacts with root-relative artifactPath and sha256","recompute Ingredient Intelligence after direct catalog finalization","run apply script twice","compare owned bytes and hashes"],secondRunDiff:"none",hashes,counts:{publishedProducts:48,normalizedMatches:27,stale:1,reconciliations:28,directPositiveProducts:3,mayContainProducts:0,matrixSearches:4},firstRunGeneratedHash:before});
console.log(JSON.stringify({fingerprint,changedPaths:[...owned,paths.apply],secondRunDiff:"none",counts:{publishedProducts:48,normalizedMatches:27,stale:1,reconciliations:28,directPositiveProducts:3,mayContainProducts:0,matrixSearches:4}},null,2));
